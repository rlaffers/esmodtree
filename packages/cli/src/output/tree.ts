...
export function formatTree(
  nodeOrForest: TreeNode | TreeNode[],
  options: FormatTreeOptions = {},
): string {
  const { color = false } = options

  function walkOne(node: TreeNode): string {
    const lines: string[] = []
  ...
  if (Array.isArray(nodeOrForest)) {
    return nodeOrForest.map(walkOne).join('\n')
  }
  return walkOne(nodeOrForest)
}
